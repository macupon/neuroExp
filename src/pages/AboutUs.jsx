  ...
  return (
    <div className="py-8">
      ...
      <section className="flex flex-col md:flex-row items-center gap-8">
        ...
        />
        <div>
          <h1 className="bg-sky-200 inline font-bold text-3xl md:text-4xl lg:text-5xl">
            Hello everyone!
          </h1>
      ...
      <section className="mt-10">
        ...
        <p className="text-gray-500 mt-1">{florencia.role}</p>
          ...
          {florencia.highlights.map((h) => {
            ...
            return (
              ...
              >
                ...
                <p className="font-light text-sm leading-6 mt-2 text-gray-600">
                  {h.text}
                </p>
                {h.link && (
                  <a
                ...
                {h.foundations && (
                  <ul className="mt-2 space-y-1">
                    {h.foundations.map((f) => (
                      <li key={f.url} className="text-sm text-gray-600 font-light">
                        <a
                          href={f.url}
                          target="_blank"
                          rel="noopener noreferrer"
      ...
      <section className="mt-12">
        <h2 className="bg-sky-200 inline font-bold text-2xl md:text-3xl">
          Our team
        </h2>
        <div className="mt-6 flex flex-col gap-6">
          {teamMembers.map((member) => (
            <div
              key={member.name}
              className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row gap-10 items-center"
            >
              ...
              <div>
                {member.bio.map((paragraph, i) => (
                  <p
                    key={i}
                    className="font-light text-sm leading-7 mt-3 text-gray-600"
                  >
                    {paragraph}
                  </p>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          {collaborators.map((collab) => (
            ...
            >
              ...
              <p className="text-gray-500 text-sm">{collab.role}</p>
              <p className="font-light text-sm leading-6 mt-3 text-gray-600">
                {collab.description}
              </p>
              <a
